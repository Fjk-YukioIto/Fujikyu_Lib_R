...
#Region "列ﾍｯﾀﾞ"
    ...
    ''' <summary>
    ''' 列ヘッダのカスタマイズを適用するか判定する
    ''' </summary>
    ''' <returns></returns>
    <Category("列ヘッダのカスタマイズ")>
    <Description("列ヘッダのカスタマイズの適用を決定します")>
...
#End Region
    ...
    Private _cellUnionMode As UnionMode
    ''' <summary>
    ''' 結合モードの設定
    ''' </summary>
    ''' <returns></returns>
    <Category("セル結合")>
    <Description("セル結合の方法を設定します")>
    Public Property CellUnionMode As UnionMode
        Get
            Return _cellUnionMode
        End Get
        Set(value As UnionMode)
...
#Region "構造体"
    ''' <summary>
    ''' 結合設定：Auto/自動　User/設定をユーザが渡す　None/結合しない
    ''' </summary>
    Enum UnionMode
        Auto = 0
        User = 1
...
#Region "ｲﾍﾞﾝﾄ"
    ...
    Protected Overrides Sub InitLayout()
        ...
        Me.CellUnionMode = UnionMode.Auto
    End Sub
